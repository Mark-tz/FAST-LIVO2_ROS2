...
void loadVoxelConfig(rclcpp::Node::SharedPtr node, VoxelMapConfig &voxel_config)
{
  ...
  node->declare_parameter("lio.layer_init_num", std::vector<int>{5,5,5,5,5});
  auto layer_init_array = node->get_parameter("lio.layer_init_num").as_integer_array();
  voxel_config.layer_init_num_.assign(layer_init_array.begin(), layer_init_array.end());
                                     ...
                                     const float alpha, const Eigen::Vector3d rgb)
{
  ...
  plane.header.stamp = rclcpp::Time(0);
  plane.ns = plane_ns;

...
// ROS1 includes (commented out for reference)
...
#include <cv_bridge/cv_bridge.h>
#include <image_transport/image_transport.hpp>  // ROS2 version
#include <nav_msgs/msg/path.hpp>  // ROS2 message format
...
#include <tf2_ros/transform_broadcaster.h>  // replaces tf
#include <tf2_ros/buffer.h>
#include <tf2_ros/transform_listener.h>
#include <vikit/camera_loader.h>  // TODO: find ROS2 alternative
...
public:
  ...
  void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr msg);  // ROS2 callback signature
  void livox_pcl_cbk(const livox_ros_driver::msg::CustomMsg::SharedPtr msg_in);  // TODO: check ROS2 livox driver
  void imu_cbk(const sensor_msgs::msg::Imu::SharedPtr msg_in);  // ROS2 callback signature
  void img_cbk(const sensor_msgs::msg::Image::SharedPtr msg_in);  // ROS2 callback signature
  void publish_img_rgb(const image_transport::Publisher &pubImage, VIOManagerPtr vio_manager);
  void publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager);  // ROS2 publisher type
  ...
  void readParameters();  // no longer needs NodeHandle parameter
  template <typename T> void set_posestamp(T &out);
  template <typename T> void pointBodyToWorld(const Eigen::Matrix<T, 3, 1> &pi, Eigen::Matrix<T, 3, 1> &po);
  template <typename T> Eigen::Matrix<T, 3, 1> pointBodyToWorld(const Eigen::Matrix<T, 3, 1> &pi);
  cv::Mat getImageFromMsg(const sensor_msgs::msg::Image::SharedPtr &img_msg);  // ROS2 message type
  ...
  deque<sensor_msgs::msg::Imu> prop_imu_buffer;  // ROS2 message type
  sensor_msgs::msg::Imu newest_imu;  // ROS2 message type
  double latest_ekf_time;
  nav_msgs::msg::Odometry imu_prop_odom;  // ROS2 message type
  rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubImuPropOdom;  // ROS2 publisher type
  ...
  deque<sensor_msgs::msg::Imu::SharedPtr> imu_buffer;  // ROS2 message type
  ...
  nav_msgs::msg::Path path;  // ROS2 message type
  nav_msgs::msg::Odometry odomAftMapped;  // ROS2 message type
  geometry_msgs::msg::Quaternion geoQuat;  // ROS2 message type
  geometry_msgs::msg::PoseStamped msg_body_pose;  // ROS2 message type
  ...
  // ROS1 publishers/subscribers (commented out for reference)
  ...
  rclcpp::TimerBase::SharedPtr imu_prop_timer;  // ROS2 timer type

  // TF2 components (replaces tf)
